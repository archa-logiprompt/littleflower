...
class Payroll_model extends CI_Model
{
    ...
    {
        $result = $this->db->select()->where('payslip_id', $id)->get('payslip_allowance')->result_array();
        return($result);
    }
    ...
    function getNetAmount($id)
    {
        ...
        $positiveAmount = 0;
        $negativeAmount = 0;
        $result = $this->db->select('net_salary')->where('id', $id)->get('staff_payslip')->row()->net_salary;
        $by = (2);
        $byAmount = $result;
        $toAmount = $result;
        $to = (14);
        $entry = array(
            ...
            'narration' => "Salary Revered by Cash",
            'centre_id' => $centre_id,
        ...
        $allocations = $this->payroll_model->getAllocations($id);
