...
//         IFNULL(clinical_attendance.id, 0) AS `attendance_id`, 
//         IFNULL(clinical_attendance.date, 'xxx') AS `date`, 
//         IFNULL(clinical_attendance.attendence_type_id, 0) AS `attendance_type_id`, 
//         IFNULL(clinical_attendance.group_id, 0) AS `group_id` 
//     FROM 
//         clinical_group 
//     INNER JOIN 
//         student_session ON student_session.id = clinical_group.student_session_id 
//     INNER JOIN 
//         students ON students.id = student_session.student_id 
//     LEFT JOIN 
//         clinical_attendance ON clinical_attendance.student_session_id = student_session.id 
//         AND clinical_attendance.date LIKE CONCAT(" . $this->db->escape($date) . ", '%') 
//         AND clinical_attendance.subject_id = '3' 
//     WHERE 
//         clinical_group.class_id = '1' 
//         AND clinical_group.section_id = '44' 
//         AND clinical_group.group_id = '23' 
//     GROUP BY 
//         clinical_group.student_session_id";
...
// 		return $query->result_array();
	...
	{
		$result = $this->db
    ->select('students.id AS `student_id`, students.admission_no, students.firstname, students.lastname, students.roll_no,student_session.id as student_session_id')
    ->from('clinical_group')
    ->join('student_session', 'student_session.id = clinical_group.student_session_id')
    ->join('students', 'students.id = student_session.student_id')
    ->where('clinical_group.group_id', $group_id)
    ->where('clinical_group.class_id', $class_id)
    ->where('clinical_group.section_id', $section_id)
    ->get()
    ->result_array();
...
return $result; 
	...
	{
		$sql = "SELECT * FROM clinical_attendance WHERE subject_id = " . $this->db->escape($subject_id) . " AND group_id = " . $this->db->escape($groupid) . " AND date LIKE '" . $this->db->escape_like_str($date) . "%'";
	...
	public function getAllStudents($class_id, $section_id, $date, $subject_id)
	{
		$sql = "SELECT students.id as `student_id`, student_session.id as `student_session_id`,students.admission_no,students.firstname,students.lastname,students.roll_no,IFNULL(clinical_attendance.id,0) as `attendance_id`,IFNULL(clinical_attendance.date,'xxx') as `date`,IFNULL(clinical_attendance.attendence_type_id,0) as `attendance_type_id` ,IFNULL(clinical_attendance.group_id,0) as `group_id` FROM student_session INNER JOIN students on students.id=student_session.student_id LEFT JOIN clinical_attendance on clinical_attendance.student_session_id=student_session.id and clinical_attendance.date=" . $this->db->escape($date) . "and clinical_attendance.subject_id=" . $this->db->escape($subject_id) . " WHERE student_session.class_id=" . $this->db->escape($class_id) . " and student_session.section_id=" . $this->db->escape($section_id) . 'group BY student_session.id';
	...
	public function getClinicalAttendance($date, $student_id, $subject_id)
	{
		$wherearr = array(
		...
		$result = $this->db->select('attendence_type_id')->where($wherearr)->get('clinical_attendance')->row()->attendence_type_id;
		// echo $this->db->last_query();
		// var_dump($date." ".$result);
		return $result;
	}

	public function getClinicalAttendanceHours($date, $student_id, $subject_id)
	{
		$wherearr = array(
			'date LIKE' => $date . '%',
			'student_session_id' => $student_id,
		...
		$this->db->select('SUM(total_minutes) as total_minutes_sum');
		$this->db->where($wherearr);
		$result = $this->db->get('clinical_attendance')->row_array();
		return $result['total_minutes_sum'];
