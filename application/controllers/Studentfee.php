...
class Studentfee extends Admin_Controller
{
        ...
        if (!empty($invoice_id)) {
            ...
            if ($paymode == 'Cash') {
                ...
                $entry = array(
                    ...
                    'narration' => "Fee has been Reverted by cash",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );


            } else if ($paymode == 'Bank' || $paymode == 'Cheque' || $paymode == 'DD') {
                $by = (8);
                $byAmount = ($amount);
                $toAmount = ($amount);
                $to =(7);
                $entry = array(
                    ...
                    'credit_amount' => json_encode(array($toAmount)),
                    'credit' => json_encode(array($to)),
                    'narration' => "Fees been Reverted by Bank",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );
                ...
                $entry = array(
                    ...
                    'credit_amount' => json_encode(array($toAmount)),
                    'credit' => json_encode(array($to)),
                    'narration' => "Fees been Reverted by Advance",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );


            } else if ($paymode == 'Excess') {
                $by = (8);
                $byAmount = ($amount);
                $toAmount = ($amount);
                $to = (10);
                $entry = array(
                    ...
                    'credit_amount' => json_encode(array($toAmount)),
                    'credit' => json_encode(array($to)),
                    'narration' => "Fees been Reverted by Excess",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );
    ...
    function refund_fee()
    {
        ...
        if ($payment_mode == 'Cash') {
            $by = (8);
            $byAmount = ($refund_amount);
            $toAmount = ($refund_amount);
            $to = (2);
            $entry = array(
                'debit' =>json_encode( array($by)),
                'debit_amount' =>json_encode(array($byAmount)),
                'credit_amount' => json_encode(array($toAmount)),
                'credit' => json_encode(array($to)),
                'narration' => "Fees Received by Cash",
                'centre_id' => $centre_id,
                'financial_year' => $financial_year
            );


        } else if ($payment_mode == 'Bank' || $payment_mode == 'Cheque' || $payment_mode == 'DD') {
            $by = (8);
            $byAmount = ($refund_amount);
            $toAmount = ($refund_amount);
            $to = (7);
            $entry = array(
                ...
                'credit_amount' => json_encode(array($toAmount)),
                'credit' => json_encode(array($to)),
                'narration' => "Fees Received to Bank",
                'centre_id' => $centre_id,
                'financial_year' => $financial_year
            );
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($payment_mode == 'Cash') {
                ...
                $entry = array(
                    ...
                    'credit_amount' => json_encode(array($toAmount)),
                    'credit' => json_encode(array($to)),
                    'narration' => "Fees Received by Advance",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );


            } else if ($payment_mode == 'Excess') {
                $by = (10);
                $byAmount =($finalamount);
                $toAmount = ($finalamount);
                $to = (8);
                $entry = array(
                    ...
                    'credit_amount' => json_encode(array($toAmount)),
                    'credit' => json_encode(array($to)),
                    'narration' => "Fees Received by Excess",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );
    ...
    function fee_advance()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($payment_mode == 'Cash') {
                $by = 2;
                $byAmount = $this->input->post('ad_amount');
                $toAmount = $this->input->post('ad_amount');
                $to = 9;
                $entry = array(
                    'debit' =>json_encode( $by),
                    'debit_amount' =>json_encode($byAmount),
                    'credit_amount' => json_encode($toAmount),
                    'credit' => json_encode($to),
                    'narration' => "Advance Amount Received by Cash",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );
                ...
                $entry = array(
                    ...
                    'credit_amount' => json_encode($toAmount),
                    'credit' => json_encode($to),
                    'narration' => "Advance Amount Received to Bank",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($payment_mode == 'Cash') {
                ...
                $entry = array(
                    ...
                    'credit_amount' => json_encode(array($toAmount)),
                    'credit' => json_encode(array($to)),
                    'narration' => "Excess Amount Received by Cash",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );
                ...
                $entry = array(
                    ...
                    'credit_amount' => json_encode(array($toAmount)),
                    'credit' => json_encode(array($to)),
                    'narration' => "Excess Amount Received to Bank",
                    'centre_id' => $centre_id,
                    'financial_year' => $financial_year
                );
