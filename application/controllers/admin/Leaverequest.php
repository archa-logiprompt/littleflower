    ...
    public function add_staff_leave()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty ($request_id)) {
                ...
                $data = array('staff_id' => $staff_id, 'date' => date("Y-m-d", strtotime($applied_date)), 'leave_days' => $leave_days, 'leave_type_id' => $leavetype, 'leave_from' => $leavefrom, 'leave_to' => $leaveto, 'employee_remark' => $reason, 'status' => $status, 'applied_by' => $applied_by, 'leave_method' => $leave_method, 'document_file' => $document);
            }
