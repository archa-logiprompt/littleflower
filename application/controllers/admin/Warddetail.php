...
class Warddetail extends Admin_Controller
{
    ...
    function index()
    {
        ...
        $location = $this->db->get('clinical_location')->result_array();
        $data['departmentlist'] = $deptname;
        $data['locations'] = $location;

        $warddetail = $this->db->select('*,clinical_department.id as wardid,warddetail.id as detailid')->join('clinical_location',"clinical_location.id=warddetail.location_id")->join('clinical_department', 'warddetail.deptnames=clinical_department.id')->get('warddetail')->result_array();
